...
namespace KeyphraseExtraction.KEUtilities
{
	...
	{
        private static StopWordsHandler _instance;
		...
		public bool ContainStopword(string str, out string stopWord)
		{
            bool hasStopWord = false;
            Regex r = new Regex("([ \\t{}():;. \n])");
            str = str.ToLower();
            stopWord = "";
            String[] tokens = r.Split(str);

            if (_stopwords.ContainsKey(tokens[0]))
            {
                hasStopWord = true;
                stopWord = tokens[0];
            }
            else if (_stopwords.ContainsKey(tokens[tokens.Length - 1]))
            {
                hasStopWord = true;
                stopWord = tokens[tokens.Length - 1];
            }
        ...
        {
            if (_stopwords == null || _instance == null)
			{
                _instance = new StopWordsHandler();
